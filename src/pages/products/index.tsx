...
const products = () => {
  ...
  const handleChange = (e: any, categoryName: string) => {
    ...
    if (categorias[0].isvalid) {
      ...
      states.forEach((s) => {
        if (!s.isvalid) {
          if (s.nome !== 'Todas') allMarked = false
        }
      })
      if (allMarked) {
                ...
                prod.produtos.nome
                  ...
                  <CardProduct
                    ...
                    onChange={() =>
                      addProduct(
                        prod,
                        quantidade[index] ? quantidade[index] : 1
                      )
                    }
                    handleRemove={() => removeProduct(prod)}
                    PlusQuantityOnChange={() =>
